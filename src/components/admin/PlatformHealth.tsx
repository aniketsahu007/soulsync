...
export const PlatformHealth = memo(({ healthData }: PlatformHealthProps) => {
  return (
    <div id="health" className="mt-12 grid grid-cols-1 lg:grid-cols-4 gap-8 scroll-mt-32">
      <Card className="lg:col-span-3 overflow-hidden rounded-[3rem] border-white bg-white dark:bg-slate-950 p-10 shadow-md dark:shadow-none ring-1 ring-slate-200/50">
        <div className="flex items-center justify-between mb-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-2xl font-black tracking-tight text-slate-900 dark:text-slate-50">Infrastructure Integrity</h3>
              <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Real-time status of global security protocols.</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {healthData.map((item, i) => (
            <div key={i} className="p-6 rounded-[2rem] bg-slate-50/50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 space-y-4">
      ...
      <Card className="overflow-hidden rounded-[3rem] bg-navy p-8 text-white shadow-xl dark:shadow-none shadow-primary/10 border border-white/10 relative">
        ...
        <Button variant="hero" className="w-full bg-white dark:bg-slate-950 text-navy hover:bg-slate-100 dark:bg-slate-900 border-none h-12 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 relative z-10">
          Download Report
        </Button>
